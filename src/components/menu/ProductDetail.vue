...
<template>
  <div v-if="product" class="flex flex-col md:flex-row -mx-6 -my-5 md:h-[560px]">
    <!-- Left: Image -->
    <div class="w-full md:w-2/5 h-64 md:h-full bg-zinc-100 overflow-hidden shrink-0 p-4">
      <img
      ...
      <div class="flex-1 overflow-y-auto p-8 space-y-6">
        ...
        <div class="flex items-center justify-between">
          ...
          <span
            ...
            :class="product.isAvailable ? 'bg-emerald-50' : 'bg-red-50'"
          >
            <span
              class="text-[11px] font-semibold"
              :class="product.isAvailable ? 'text-emerald-600' : 'text-red-600'"
            >
        ...
        <!-- Name & Price -->
        <div>
          <h2 class="text-xl font-bold text-zinc-900 leading-snug">{{ product.name }}</h2>
          <p class="mt-2 text-3xl font-bold text-amber-700 tabular-nums">{{ resolvedPrice }}</p>
        </div>
        ...
        <template v-if="product.optionSets?.length > 0">
          <div
            ...
            class="space-y-3"
          >
            <div v-if="index > 0" class="h-px bg-zinc-100" />
        ...
        <template v-else>
          ...
          <div
            class="flex items-start gap-3 rounded-lg bg-zinc-50 border border-zinc-100 px-4 py-3.5"
          >
            <span
              class="material-symbols-outlined text-[18px] text-zinc-400 mt-px shrink-0"
              style="font-variation-settings: 'FILL' 0"
              >info</span
            >
            <div>
              <p class="text-sm font-semibold text-zinc-700">No customisation options</p>
  ...
  <div v-else class="flex items-center justify-center py-8">
    <p class="text-zinc-500">Loading product details...</p>
  </div>
</template>
